...
const App2 = () => {
  ...
  useEffect(() => {
    ...
    const fetchSearchHistory = async()=>{
        const usersHistory = await axios.get(`${backend_url}/api/v1/get-users-search-history`);
  ...
  const handleFetchWeatherDetails = async () => {
    ...
    try {
      const response = await axios.get(`${backend_url}/api/v1/getweatherdetails`, {
        params: {
    ...
    <div className="bg-zinc-800 min-h-screen w-full text-white p-8 2xl:p-60">
      ...
      {weatherData && (
        <div className="bg-gray-900 p-6 rounded-lg shadow-lg">
          <h2 className="text-2xl font-bold mb-4">{weatherData.name}, {weatherData.country}</h2>
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 bg-gray-800 rounded-md shadow">
              <p className="text-lg">Temperature: <span className="font-bold">{weatherData.temprature}°C</span></p>
      ...
      <div className="mt-4">
        {searchHistory.map((val,ind)=>(
            <div className="flex flex-col items-center space-y-2 sm:justify-around sm:items-baseline sm:flex-row  rounded-md py-2 bg-gray-900 text-gray-200 my-1" key={ind}>
                ...
                <button onClick={()=>{
                    const weatherData = val.result;
                      ...
                      setWeatherData(weatherDetails);
                }} className="hover:text-white">View results</button>
            </div>
        ))}
      </div>
    </div>
